...
/*   Updated: 2023/03/02 20:57:34 by jcervoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::map<std::string, Channel>::iterator it = serv->all_channels.find(&entries[1][1]);
	if (it != serv->all_channels.end())
		it->second.cmd_lexer(*this, serv);
	else
...
{
	map<string, Channel>::iterator it_cha;
	size_t i = 0;
	...
	if (reply == "UNDEFINED")
	{
		...
		while (i < nb_chan)
		{
			...
			{
				it_cha->second.part(*this, serv);
				if (it_cha->second.getOnlineCount() == 0)
...
int Request::kick(Server *serv)
{
	...
	if (check_lists() == 0)
	{
		...
		if (reply == "UNDEFINED")
		{
			...
			while (i < nb_chan)
			{
				it_cha = serv->all_channels.find(&entries[i][1]);
				if (it_cha == serv->all_channels.end())
					reply = errNoSuchChannel(origin);
				else
				{
				...
				while (i < entries.size() && nb_chan != 0)
				{
					...
					if (it_cha != serv->all_channels.end())
					{
						if (it_cha->second.activeMode('s') == false)
							it_cha->second.cmd_lexer(*this, serv);
						reply += rpl_endofnames(it_sender->second.setPrefix(), it_cha->first);
					}
					i++;
				}
			}
